REM ORIGINAL BY MR MAGNALAL EDITED BY MR KAGISO MAJA 
REM Computer Science 3A GRAPH PROJECT
...
set JAVAFX_MODULES=javafx.base,javafx.controls,javafx.fxml,javafx.graphics,javafx.media,javafx.swing
set JAVAFX_ARGS=
...
REM Compile project by compiling just CrimeSceneAnalysisSystem. CrimeSceneAnalysisSystem will reference required classes.
:COMPILE
echo ~~~ Compiling project ~~~
javac %JAVAFX_ARGS% -sourcepath %PRAC_SRC% -cp %PRAC_BIN%;%PRAC_LIB% -d %PRAC_BIN% %PRAC_SRC%\CrimeSceneAnalysisSystem.java
IF /I "%ERRORLEVEL%" NEQ "0" (
    set ERRMSG=~~! Error compiling project !~~
    GOTO ERROR
)

REM Generate JavaDoc for project for only crimescene subpackage.
:JAVADOC
echo ~~~ Generate JavaDoc for project ~~~
javadoc %JAVAFX_ARGS% -sourcepath %PRAC_SRC% -classpath %PRAC_BIN%;%PRAC_LIB% -use -version -author -d %PRAC_DOC%\%PRAC_JDC% -subpackages crimescene
IF /I "%ERRORLEVEL%" NEQ "0" (
    echo ~~! Error generating JavaDoc for project !~~
)

REM Run project by running CrimeSceneAnalysisSystem. Any arguments passed to the batch file are passed to CrimeSceneAnalysisSystem.
:RUN
echo ~~~ Running project ~~~
java %JAVAFX_ARGS% -cp %PRAC_BIN%;%PRAC_LIB% CrimeSceneAnalysisSystem %*
IF /I "%ERRORLEVEL%" NEQ "0" (
